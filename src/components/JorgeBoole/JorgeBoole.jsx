...
export default function JorgeBoole() {
    ...
    const calcularIntegral = (e) => {
        ...
        if (nNum < 4 || nNum % 4 !== 0) {
            alert("El número de subintervalos (n) debe ser múltiplo de 4 y mayor o igual a 4.");
            return;
        }
        ...
        for (let i = 0; i <= nNum; i++) {
            ...
            const contribucion = coeficiente * fx * (2 * h / 45);
        ...
        <div className='flex flex-row gap-6'>
            <div className='flex flex-col gap-8 w-[70%]'>
                ...
                <form onSubmit={calcularIntegral} className="grid grid-cols-2 gap-6">
                    ...
                    <div>
                        <label htmlFor="n" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Número de Subintervalos (n)</label>
                        <input type="number" id="n" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-[#292929] dark:border-[#404040] dark:placeholder-gray-400 dark:text-white" placeholder="4" required value={n} onChange={e => setN(Number(e.target.value))} />
                    </div>
                    <div className="flex items-center gap-4">
                        <button type="submit" className="bg-[#006fff] hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center text-white transition dark:bg-[#0085ff] dark:hover:bg-blue-600 dark:focus:ring-blue-800">Calcular</button>
                ...
                <div className="relative overflow-x-auto">
                    ...
                    <h2 className="text-xl font-bold mt-4">Puntos para Graficar</h2>
            ...
            <div className='bg-[#f9fafb] dark:bg-[#353535] p-5 rounded-lg w-[30%] h-[95vh]'>
                ...
                <p className="my-2 text-lg dark:text-neutral-300">La integral de la función desde {a} hasta {b} es aproximadamente {resultado ? resultado.toFixed(2) : "NaN"}.</p>
            </div>
        </div>
    )
}
