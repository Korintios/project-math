...
export default function SimpsonSimple() {
    const [funcion, setFuncion] = useState('');
    const [a, setA] = useState('');
    const [b, setB] = useState('');
    const [n, setN] = useState(2); // Debe ser par para el método Simpson Simple
    ...
    const calcularIntegral = (e) => {
        e.preventDefault();
        if (n % 2 !== 0) {
            alert("El número de subintervalos (n) debe ser par.");
            return;
        }
        const aNum = Number(a);
        const bNum = Number(b);
        ...
        for (let i = 0; i <= nNum; i++) {
            ...
            const fx = evaluate(funcion.replace(/x/g, `(${x})`).replace(/ln/g, 'log'));
